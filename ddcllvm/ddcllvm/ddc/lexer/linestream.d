...
public enum LineEnding : int {
    ...
    CRLF,
    /// LF followed by CR (0x0A, 0x0D) - rarely used
    LFCR,
    /// CR (0x0D) - mac style
    CR,
    /// unknown line ending
...
version (Windows) {
    const LineEnding DEFAULT_LINE_ENDING = LineEnding.CRLF;
} else version (OSX) {
    const LineEnding DEFAULT_LINE_ENDING = LineEnding.CR;
} else {
    const LineEnding DEFAULT_LINE_ENDING = LineEnding.LF;
}
...
struct TextFileFormat {
    ...
    /// line ending style
    LineEnding lineEnding = DEFAULT_LINE_ENDING;
    /// byte order mark character flag
    bool bom = false;
    /// true if line endings are mixed
    bool mixedLineEndings = false;
    /// error code
    SourceError errorCode = SourceError.None;
...
enum SourceError : ubyte {
    None, // no error
    WrongEncoding, // invalid encoding (e.g. bytes are not valid utf characters)
    UnexpectedEof, // file is not read completely - last char is incomplete
    FileNotFound
}

private void decodeSourceBytesUtf8(ubyte[] buf, ref char[] res, ref TextFileFormat fmt) {
    ...
    if (pos < buf.length && fmt.errorCode == SourceError.None)
        fmt.errorCode = SourceError.UnexpectedEof;
    res.length = pos;
}

private void decodeSourceBytesUtf32be(ubyte[] buf, ref char[] res, ref TextFileFormat fmt) {
    if (buf.length % 4 != 0) {
        fmt.errorCode = SourceError.UnexpectedEof;
        buf.length = buf.length & 0xFFFFFFFC;
    }
    int pos = 0;
    res.length = buf.length / 4 + 64;
    char[4] d;
    for (int i = 0; i < buf.length; i += 4) {
        uint ch1 = cast(uint)buf[i];
        uint ch2 = cast(uint)buf[i + 1];
        uint ch3 = cast(uint)buf[i + 2];
        uint ch4 = cast(uint)buf[i + 3];
        dchar ch = (ch1 << 24) | (ch2 << 16) | (ch3 << 8) | ch4;
        if (ch >= 0x100000) {
            fmt.errorCode = SourceError.WrongEncoding;
            break;
        }
        if (res.length < pos + 4)
            res.length = res.length + 4096;
        int bytes = cast(int)encode(d, ch);
...
private void decodeSourceBytesUtf32le(ubyte[] buf, ref char[] res, ref TextFileFormat fmt) {
    if (buf.length % 4 != 0) {
        fmt.errorCode = SourceError.UnexpectedEof;
        buf.length = buf.length & 0xFFFFFFFC;
    }
    ...
    char[4] d;
    for (int i = 0; i < buf.length; i += 4) {
        uint ch1 = cast(uint)buf[i + 3];
        uint ch2 = cast(uint)buf[i + 2];
        uint ch3 = cast(uint)buf[i + 1];
        uint ch4 = cast(uint)buf[i + 0];
        dchar ch = (ch1 << 24) | (ch2 << 16) | (ch3 << 8) | ch4;
        if (ch >= 0x100000) {
            fmt.errorCode = SourceError.WrongEncoding;
            break;
...
private void decodeSourceBytesUtf16be(ubyte[] buf, ref char[] res, ref TextFileFormat fmt) {
    if (buf.length % 2 != 0) {
        fmt.errorCode = SourceError.UnexpectedEof;
        buf.length = buf.length & 0xFFFFFFFE;
    }
    int pos = 0;
    res.length = buf.length / 2 + 64;
    char[4] d;
...
void decodeSourceBytesUtf16le(ubyte[] buf, ref char[] res, ref TextFileFormat fmt) {
    if (buf.length % 2 != 0) {
        fmt.errorCode = SourceError.UnexpectedEof;
        buf.length = buf.length & 0xFFFFFFFE;
    }
    ...
    char[4] d;
    for (int i = 0; i < buf.length; i += 2) {
        uint ch1 = cast(uint)buf[i + 1];
        uint ch2 = cast(uint)buf[i];
        dchar ch = (ch1 << 8) | ch2;
        if (ch >= 0x100000) {
            fmt.errorCode = SourceError.WrongEncoding;
            break;
...
void convertLineEndings(ref char[] res, ref TextFileFormat fmt) {
    int src = 0;
    int dst = 0;
    int crcount = 0;
    int lfcount = 0;
    ...
    if (crcount > 0 && lfcount == 0)
        fmt.lineEnding = LineEnding.CR;
    else if (crcount == 0 && lfcount != 0)
        fmt.lineEnding = LineEnding.LF;
    else if (crcount > 0 && lfcount > 0) {
        if (crcount == lfcount) {
            if (lfcrcount > crlfcount)
                fmt.lineEnding = LineEnding.LFCR;
            else
                fmt.lineEnding = LineEnding.CRLF;
        } else {
            fmt.lineEnding = DEFAULT_LINE_ENDING;
            fmt.mixedLineEndings = true;
        }
    }
    res.length = dst;
}

/// convert source bytes into utf8 with LF line endings, autodetect encoding and line ending
TextFileFormat decodeSourceBytes(ubyte[] buf, ref char[] res) {
    TextFileFormat fmt;
    if (buf.length == 0) {
        res.length = 0;
        return fmt;
    ...
    if (buf.length >= 3 && buf[0] == 0xEF && buf[1] == 0xBB && buf[2] == 0xBF) {
        fmt.bom = true;
        fmt.encoding = EncodingType.UTF8;
        decodeSourceBytesUtf8(buf[3 .. $], res, fmt);
    } else if (buf.length >= 4 && buf[0] == 0x00 && buf[1] == 0x00 && buf[2] == 0xFE && buf[3] == 0xFF) {
        fmt.bom = true;
        fmt.encoding = EncodingType.UTF32BE;
        decodeSourceBytesUtf32be(buf[4 .. $], res, fmt);
    } else if (buf.length >= 4 && buf[0] == 0xFF && buf[1] == 0xFE && buf[2] == 0x00 && buf[3] == 0x00) {
        fmt.bom = true;
        fmt.encoding = EncodingType.UTF32BE;
        decodeSourceBytesUtf32le(buf[4 .. $], res, fmt);
    } else if (buf.length >= 2 && buf[0] == 0xFE && buf[1] == 0xFF) {
        fmt.bom = true;
        fmt.encoding = EncodingType.UTF16BE;
        decodeSourceBytesUtf16be(buf[2 .. $], res, fmt);
    } else if (buf.length >= 2 && buf[0] == 0xFF && buf[1] == 0xFE) {
        fmt.bom = true;
        fmt.encoding = EncodingType.UTF16LE;
        decodeSourceBytesUtf16le(buf[2 .. $], res, fmt);
    } else if (buf.length >= 4 && buf[0] == 0x00 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] != 0x00) {
        // autodetected UTF32BE w/o BOM
        fmt.encoding = EncodingType.UTF32BE;
        decodeSourceBytesUtf32be(buf, res, fmt);
    } else if (buf.length >= 4 && buf[0] != 0x00 && buf[1] == 0x00 && buf[2] == 0x00 && buf[3] == 0x00) {
        // autodetected UTF32LE w/o BOM
        fmt.encoding = EncodingType.UTF32BE;
        decodeSourceBytesUtf32le(buf, res, fmt);
    } else if (buf.length >= 2 && buf[0] == 0x00 && buf[1] != 0x00) {
        // autodetected UTF16BE w/o BOM
        fmt.encoding = EncodingType.UTF16BE;
        decodeSourceBytesUtf16be(buf, res, fmt);
    } else if (buf.length >= 2 && buf[0] != 0x00 && buf[1] == 0x00) {
        // autodetected UTF16LE w/o BOM
        fmt.encoding = EncodingType.UTF16LE;
        decodeSourceBytesUtf16le(buf, res, fmt);
	} else {
        fmt.encoding = EncodingType.UTF8;
        decodeSourceBytesUtf8(buf, res, fmt);
    }
    convertLineEndings(res, fmt);
    return fmt;
}
...
class TextLines : SourceFile {
    ...
    string rangeText(int startLine, int startPos, int endLine, int endPos) const {
        ...
        int endOffset = endLineOffset + endPos;
